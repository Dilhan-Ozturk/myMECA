...
def mecha(general_config='./extdata/Maize_General.xml',#'Arabido1_General.xml' #'MilletLR3_General.xml' #
    ...
    n_xylem_cells=len(network.xylem_cells)
    ...
    lat_dists=zeros((network.n_walls,1))
    Nmb=0 #Total number of membranes
    cellperimeter=np.linspace(0,0,network.n_cells)
    cellarea=np.linspace(0,0,network.n_cells) #(micron^2)
    ...
    for cell_group in network.cellset['cell_to_wall']: #Loop on cells. network.cellset['cell_to_wall'] contains cell wall groups info (one group by cell)
        cellnumber1 = int(cell_group.getparent().get("id")) #Cell ID number
        i=0
        for r in cell_group: #Loop for wall elements around the cell
            wall_id= int(r.get("id")) #Cell wall ID
            d_vec=array([position[wall_id][0]-position[network.n_wall_junction+cellnumber1][0],position[wall_id][1]-position[network.n_wall_junction+cellnumber1][1]])
            dist_cell=hypot(d_vec[0],d_vec[1]) #distance between wall node and cell node (micrometers)
            if dist_cell==0:
                print(dist_cell,cellnumber1)
            d_vec/=dist_cell
            lat_dists[wall_id]+=dist_cell
            if i==0:
                wid0=wall_id
                wid1=wall_id
            else: #This algorithm only works if walls are ordered anti-clockwise around the cell center
                wid2=wall_id #new wall id
                #Find junction closest to wid1
                dist1=hypot(position[wid1][0]-network.junction_positions[wid2][0],position[wid1][1]-network.junction_positions[wid2][1])
                dist2=hypot(position[wid1][0]-network.junction_positions[wid2][2],position[wid1][1]-network.junction_positions[wid2][3])
                if dist1<dist2:
                    j=0
                else:
                    j=2
                cellarea[cellnumber1] += (position[wid1][0]+network.junction_positions[wid2][0+j])*(position[wid1][1]-network.junction_positions[wid2][1+j]) #Cell area loop (micron^2)
                cellarea[cellnumber1] += (network.junction_positions[wid2][0+j]+position[wid2][0])*(network.junction_positions[wid2][1+j]-position[wid2][1]) #Cell area loop (micron^2)
                wid1=wid2
            Nmb+=1
            network.graph.add_edge(network.n_wall_junction + cellnumber1, wall_id, path='membrane', length=network.wall_lengths[wall_id], dist=dist_cell, d_vec=d_vec) #, height=height #Adding all cell to wall connections (edges) #kaqp=kaqp, kw=kw, kmb=kmb, 
            cellperimeter[cellnumber1]+=network.wall_lengths[wall_id]
            i+=1
        dist1=hypot(position[wid1][0]-network.junction_positions[wid0][0],position[wid1][1]-network.junction_positions[wid0][1])
        dist2=hypot(position[wid1][0]-network.junction_positions[wid0][2],position[wid1][1]-network.junction_positions[wid0][3])
        if dist1<dist2:
            j=0
        else:
            j=2
        cellarea[cellnumber1] += (position[wid1][0]+network.junction_positions[wid0][0+j])*(position[wid1][1]-network.junction_positions[wid0][1+j]) #Back to the first node
        cellarea[cellnumber1] += (network.junction_positions[wid0][0+j]+position[wid0][0])*(network.junction_positions[wid0][1+j]-position[wid0][1]) #Back to the first node
        cellarea[cellnumber1] /= -2.0
    
    Cell_connec=-ones((network.n_cells,65),dtype=int) #Connected cells for further ranking
    nCell_connec=zeros((network.n_cells,1),dtype=int) #Quantity of cell to cell connectionsC:\Users\heymansad
    for i in range(0, len(network.cellset['walls'])): #Loop on walls, by cell - wall association, hence a wall can be repeated if associated to two cells. Parent structure: Cell/Walls/Wall
        ...
        for j in range(i + 1, len(network.cellset['walls']) ): #Loop on cell-wall associations that are further down in the list
            ...
            if id1 == id2: #If walls 1 and 2 are the same, then cells 1 and 2 are connected by plasmodesmata
                d_vec=array([position[network.n_wall_junction+int(cellid2)][0]-position[network.n_wall_junction+int(cellid1)][0],position[network.n_wall_junction+int(cellid2)][1]-position[network.n_wall_junction+int(cellid1)][1]])
                dist_cell=hypot(d_vec[0],d_vec[1]) #distance between wall node and cell node (micrometers)
                d_vec/=dist_cell
                network.graph.add_edge(network.n_wall_junction + int(cellid1), network.n_wall_junction + int(cellid2), path='plasmodesmata', length=network.wall_lengths[int(id1)], d_vec=d_vec) #, height=height #Adding all cell to cell connections (edges) #kpl=kpl, 
    ...
    network.n_junctions=0
    for Junction, Walls in network.junction_to_wall.items(): #Loop on junctions between walls
        for wall_id in Walls: #Walls is the list of cell walls ID meeting at the junction pos
            if (network.n_junctions + network.n_walls) not in position:
                print(f"Error: Key {network.n_junctions + network.n_walls} is missing from 'position' dictionary.")
            d_vec=array([position[wall_id][0]-position[network.n_junctions+network.n_walls][0],position[wall_id][1]-position[network.n_junctions+network.n_walls][1]])
            ...
            network.graph.add_edge(network.n_junctions+network.n_walls, int(wall_id), path='wall', length=network.wall_lengths[int(wall_id)]/2, lat_dist=lat_dists[int(wall_id)][0], d_vec=d_vec, dist_wall=dist_wall) #Adding junction to wall connections (edges)
        network.n_junctions+=1
    ...
    for cell_group in network.cellset['cell_to_wall']: #Loop on cells. network.cellset['cell_to_wall'] contains cell wall groups info (one group by cell)
        cellnumber1 = int(cell_group.getparent().get("id")) #Cell ID number
        if network.graph.nodes[network.n_wall_junction + cellnumber1]['cgroup']==3: #Endodermis
                x_grav+=position[network.n_wall_junction + cellnumber1][0]
                y_grav+=position[network.n_wall_junction + cellnumber1][1]
                n_cell_endo+=1
    x_grav/=n_cell_endo
    y_grav/=n_cell_endo
    
    Cell_rank=zeros((network.n_cells,1)) #Ranking of cells (1=Exodermis, 2=Epidermis, 3=Endodermis, 4*=Cortex, 5*=Stele, 11=Phloem sieve tube, 12=Companion cell, 13=Xylem, 16=Pericycle), stars are replaced by the ranking within cortical cells and stele cells
    ...
    for cell_group in network.cellset['cell_to_wall']: #Loop on cells. network.cellset['cell_to_wall'] contains cell wall groups info (one group by cell)
        cellnumber1 = int(cell_group.getparent().get("id")) #Cell ID number
        celltype=network.graph.nodes[network.n_wall_junction + cellnumber1]['cgroup'] #Cell type
        ...
        x_cell=position[network.n_wall_junction + cellnumber1][0] #Cell position (micrometers)
        y_cell=position[network.n_wall_junction + cellnumber1][1]
    ...
    for cell_group in network.cellset['cell_to_wall']: #Loop on cells. network.cellset['cell_to_wall'] contains cell wall groups info (one group by cell)
        cellnumber1 = int(cell_group.getparent().get("id")) #Cell ID number
        #celltype=network.graph.nodes[network.n_wall_junction + cellnumber1]['cgroup']
        ...
        if celltype==4: #Cortex
            ...
            if any(Cell_rank[Cell_connec[cellnumber1][0:nCell_connec[cellnumber1][0]]]==stele_connec_rank): #Cell to cell connection with pericycle
                ...
                Layer_dist[50]+=dist
                nLayer[50]+=1
                if network.graph.nodes[network.n_wall_junction + cellnumber1]['cgroup']==11 or network.graph.nodes[network.n_wall_junction + cellnumber1]['cgroup']==23:
                    listprotosieve.append(network.n_wall_junction + cellnumber1)
    Nsieve=len(network.sieve_cells)
    ...
    for node, edges in network.graph.adjacency() :
        ...
        if i>=network.n_wall_junction: #Cell
            if network.graph.nodes[i]['cgroup']==16 or network.graph.nodes[i]['cgroup']==21:
                for neighboor, eattr in edges.items(): #Loop on connections (edges)
                    if eattr['path'] == "plasmodesmata" and (network.graph.nodes[indice[neighboor]]['cgroup']==11 or network.graph.nodes[indice[neighboor]]['cgroup']==23): #Plasmodesmata connection  #eattr is the edge attribute (i.e. connection type)
                        PPP.append(i-network.n_wall_junction)
            elif network.graph.nodes[i]['cgroup']==outercortex_connec_rank or network.graph.nodes[i]['cgroup']==4 or network.graph.nodes[i]['cgroup']==3: #exodermis or cortex or endodermis (or epidermis if there is no exodermis)
                if i-network.n_wall_junction not in geometry.intercellular_ids: #The loop focuses on exo, cortex and endodermis cells that are not intercellular spaces
                    for neighboor, eattr in edges.items(): #Loop on connections (edges)
                        if eattr['path'] == "plasmodesmata": #Plasmodesmata connection  #eattr is the edge attribute (i.e. connection type)
                            ...
                            if (network.graph.nodes[i]['cgroup']==outercortex_connec_rank and network.graph.nodes[j]['cgroup']==4) or (network.graph.nodes[j]['cgroup']==outercortex_connec_rank and network.graph.nodes[i]['cgroup']==4):#Exodermis to cortex cell or vice versa (epidermis if no exodermis exists)
                                Length_outer_cortex_tot+=l_membrane
                                if j-network.n_wall_junction not in geometry.intercellular_ids:
                                    Length_outer_cortex_nospace+=l_membrane
                            elif (network.graph.nodes[i]['cgroup']==4 and network.graph.nodes[j]['cgroup']==4):#Cortex to cortex cell
                                Length_cortex_cortex_tot+=l_membrane
                                if j-network.n_wall_junction not in geometry.intercellular_ids:
                                    Length_cortex_cortex_nospace+=l_membrane
                            elif (network.graph.nodes[i]['cgroup']==3 and network.graph.nodes[j]['cgroup']==4) or (network.graph.nodes[j]['cgroup']==3 and network.graph.nodes[i]['cgroup']==4):#Cortex to endodermis cell or vice versa
    ...
    dist_grav=zeros((network.n_walls,1)) #distance between node and cross-section center of gravity (micrometers)
    temp=0.0 #Counting the number of membranes from epidermis
    for cell_group in network.cellset['cell_to_wall']: #Loop on cells. network.cellset['cell_to_wall'] contains cell wall groups info (one group by cell)
        cellnumber1 = int(cell_group.getparent().get("id")) #Cell ID number
        for r in cell_group: #Loop for wall elements around the cell
            wall_id= int(r.get("id")) #Cell wall ID
            dist_grav[wall_id]=sqrt(square(position[wall_id][0]-x_grav)+square(position[wall_id][1]-y_grav)) #Distance between membrane and cross section centre of gravity (micrometers)
            if network.graph.nodes[network.n_wall_junction + cellnumber1]['cgroup']==4: #Cortex
                dmax_cortex=max(dmax_cortex,dist_grav[wall_id])
                dmin_cortex=min(dmin_cortex,dist_grav[wall_id])
            elif network.graph.nodes[network.n_wall_junction + cellnumber1]['cgroup']==2: #Epidermis
                davg_epi+=dist_grav[wall_id][0]
                temp+=1.0
    ...
    if general.paraview==1 or general.par_track==1 or general.apo_contagion>0 or general.sym_contagion>0:
        ...
        nThickWalls=zeros((2*network.n_walls,1)) #This will save how many new junction wall ID were already saved in connections to new wall ID (the vector is a little too long)
        ThickWallsX=[] #Same as Thickwalls except that it includes extra info about borderlink nodes, was created to display walls
        Wall2NewWall=empty((network.n_walls,2))
        Wall2NewWall[:]=np.nan
        nWall2NewWall=zeros((network.n_walls,1))
        Wall2NewWallX=empty((network.n_wall_junction,8)) #This one also includes "junction" to "new junctions" ID
        Wall2NewWallX[:]=np.nan
        nWall2NewWallX=zeros((network.n_wall_junction,1))
        ThickWallPolygonX=empty((network.n_walls*2,4))
        ThickWallPolygonX[:]=np.nan
        nThickWallPolygonX=zeros((network.n_walls*2,1))
        Wall2Cell=empty((network.n_walls,2))
        Wall2Cell[:]=np.nan
        nWall2Cell=zeros((network.n_walls,1))
        Junction2Wall2Cell=empty((network.n_wall_junction-network.n_walls,12))
        Junction2Wall2Cell[:]=np.nan
        nJunction2Wall2Cell=zeros((network.n_wall_junction-network.n_walls,1))
        Junction2Wall2=empty((network.n_wall_junction-network.n_walls,12)) #This network.junction_to_wall will be incomplete because it needs to have the same dimension as Junction2Wall2Cell
        Junction2Wall2[:]=np.nan
        nJunction2Wall2=zeros((network.n_wall_junction-network.n_walls,1))
        Wall2Junction=empty((network.n_walls,2))
        Wall2Junction[:]=np.nan
        nWall2Junction=zeros((network.n_walls,1))
        Cell2ThickWalls=empty((network.n_cells,32))
        Cell2ThickWalls[:]=np.nan
        nCell2ThickWalls=zeros((network.n_cells,1))
        r_rel=empty((network.n_walls,1))
        r_rel[:]=np.nan
        x_rel=empty((network.n_wall_junction+network.n_cells,1))
        x_rel[:]=np.nan
        L_diff=((abs(float(Layer_dist[3][0]-Layer_dist[2][0])*1.0E-04),abs(float(Layer_dist[3][0] - xyl80_dist)*1.0E-04))) #Diffusion network.wall_lengths (cm)
        twpid=0 #Thick wall point ID
        twpidX=0
        for node, edges in network.graph.adjacency():
            wall_id=indice[node]
            if wall_id<network.n_walls: #wall that is not a junction (connected to a cell)
                for neighboor, eattr in edges.items(): #Loop on connections (edges)
                    cid=int(indice[neighboor])
                    if network.graph.nodes[cid]['type']=='cell':
                        if not position[cid][0]==position[wall_id][0]:
                            slopeCG=(position[cid][1]-position[wall_id][1])/(position[cid][0]-position[wall_id][0]) #slope of the line connecting the wall node to the center of gravity of the cell
                        else:
                            slopeCG=inf
                        x=position[wall_id][0]+cos(arctan(slopeCG))*general.thickness_disp/2*sign(position[cid][0]-position[wall_id][0]) #-> new position of the wall node on the current cell side
                        y=position[wall_id][1]+sin(arctan(slopeCG))*general.thickness_disp/2*sign(position[cid][0]-position[wall_id][0])
                        ThickWalls.append(array((twpid,wall_id,cid,x,y,inf,inf,network.border_link[wall_id]))) #Adds the thick wall node ID, its parent node ID, the associated cell ID, the new coordinates in X and Y, and the two neighbouring new junction walls IDs (not known at this point in the loop)     network.graph.nodes[i]['borderlink']
                        Cell2ThickWalls[cid-network.n_wall_junction][int(nCell2ThickWalls[cid-network.n_wall_junction])]=twpid
                        nCell2ThickWalls[cid-network.n_wall_junction]+=1
                        ThickWallsX.append((twpidX,x,y,wall_id,cid)) #Adds the thick wall node ID, the new coordinates in X and Y, its parent node ID, the associated cell ID
                        ThickWallPolygonX[2*wall_id][int(nThickWallPolygonX[2*wall_id])]=twpidX #This one is for "polygon 1"
                        ThickWallPolygonX[2*wall_id+1][int(nThickWallPolygonX[2*wall_id+1])]=twpidX #This one for "polygon 2" (wall_id new thick nodes are included in two polygons)
                        nThickWallPolygonX[2*wall_id]+=1
                        nThickWallPolygonX[2*wall_id+1]+=1
                        Wall2Cell[wall_id][int(nWall2Cell[wall_id][0])]=cid
                        nWall2Cell[wall_id]+=1
                        Wall2NewWall[wall_id][int(nWall2NewWall[wall_id])]=twpid
                        nWall2NewWall[wall_id]+=1  #the count in nWall2NewWall is actually the same as nWall2Cell 
                        Wall2NewWallX[wall_id][int(nWall2NewWallX[wall_id])]=twpidX
                        nWall2NewWallX[wall_id]+=1
                        twpid+=1
                        twpidX+=1
                        if network.border_link[wall_id]==1: #network.graph.nodes[wall_id]['borderlink']==1:
                            x=position[wall_id][0]-cos(arctan(slopeCG))*general.thickness_disp/2*sign(position[cid][0]-position[wall_id][0]) #-> new position of the wall node opposite to the current cell side
                            y=position[wall_id][1]-sin(arctan(slopeCG))*general.thickness_disp/2*sign(position[cid][0]-position[wall_id][0])
                            ThickWallsX.append((twpidX,x,y,wall_id,inf))
                            ThickWallPolygonX[2*wall_id][int(nThickWallPolygonX[2*wall_id])]=twpidX #This one is for "polygon 1"
                            ThickWallPolygonX[2*wall_id+1][int(nThickWallPolygonX[2*wall_id+1])]=twpidX #This one for "polygon 2" (wall_id new thick nodes are included in two polygons)
                            nThickWallPolygonX[2*wall_id]+=1
                            nThickWallPolygonX[2*wall_id+1]+=1
                            ...
                            nWall2NewWallX[wall_id]+=1
                            twpidX+=1
                    elif network.graph.nodes[neighboor]['type']=='apo': #Node j is a junction
                        Wall2Junction[wall_id][int(nWall2Junction[wall_id][0])]=indice[neighboor]
                        nWall2Junction[wall_id]+=1
                cid1=Wall2Cell[wall_id][0]
                cid2=Wall2Cell[wall_id][1]
                rank1=Cell_rank[int(cid1-network.n_wall_junction)]
                row1=rank2row[int(rank1.item())][0]
                if not isnan(cid2):
                    rank2=Cell_rank[int(cid2-network.n_wall_junction)]
                ...
                if row1 <= rank2row[3] and row2 <= rank2row[3]: #Row on the stelar side
                    ...
                    r_rel[wall_id]=max(min(rad_pos,-0.00001),-1)
                    ...
                    r_rel[wall_id]=min(max(rad_pos,0.00001),1)
            x_rel[wall_id]=(position[wall_id][0]-network.x_min)/(network.x_max-network.x_min)
    else:
        Wall2Cell=empty((network.n_walls,2))
        Wall2Cell[:]=np.nan
        nWall2Cell=zeros((network.n_walls,1))
    ...
    if general.paraview==1 or general.par_track==1 or general.apo_contagion>0 or general.sym_contagion>0:
        for node, edges in network.graph.adjacency():
            i=indice[node]
            if i<network.n_walls: #wall that is not a junction (connected to a cell)
                for neighboor, eattr in edges.items(): #Loop on connections (edges) separated to make sure that Wall2Cell[i] is complete
                    j=indice[neighboor]
                    if network.graph.nodes[j]['type']=='apo': #then j is a junction node 
                        for cid in Wall2Cell[i]: #Wall2Cell[i][k] are the cell node ID associated to Wall i
                            if cid not in Junction2Wall2Cell[j-network.n_walls]: 
                                Junction2Wall2Cell[j-network.n_walls][int(nJunction2Wall2Cell[j-network.n_walls][0])]=cid #Writes the cells indirectly associated to each junction
                                nJunction2Wall2Cell[j-network.n_walls]+=1
                                Junction2Wall2[j-network.n_walls][int(nJunction2Wall2[j-network.n_walls])]=i #Writes the walls directly associated to each junction
                                nJunction2Wall2[j-network.n_walls]+=1  #the count in network.n_junction_to_wall is actually the same as nJunction2Wall2Cell 
                                ...
                                #Find wid1
                                for id1, val in enumerate(Junction2Wall2Cell[j-network.n_walls]): 
                                    if val==cid: #Finding the position of the current cell in the list of cells related to the junction 
                                        wid1=int(Junction2Wall2[j-network.n_walls][id1]) #At the same position in Junction2Wall2 we can find the "other" wall ID (wid1) that was already associated to the same junction and cell
                                        break
                                ...
                                ThickWalls.append(array((twpid,j,int(cid),x,y,twpid1,twpid2,network.border_link[j]))) #Adds the thick wall node ID, its parent node ID, the associated cell ID, the new coordinates in X and Y, and in this case, the two neighbouring walls that will consitute 2 neighbouring "cells" in the sense of pvtk     network.graph.nodes[j]['borderlink']
                                ...
                                Cell2ThickWalls[int(cid-network.n_wall_junction)][int(nCell2ThickWalls[int(cid-network.n_wall_junction)])]=twpid
                                nCell2ThickWalls[int(cid-network.n_wall_junction)]+=1
                                ...
                                twpid+=1
                                twpidX+=1
                                if network.border_link[j]==1: #network.graph.nodes[j]['borderlink']==1: #If the wall is at a border, there is only one cell => need to add the opposite new wall node independently
                                    if not slopeCG==0.0:
                                        slopeCG=-1/slopeCG
                                    else:
                                        slopeCG=inf
                                    ...
                                    if network.border_link[i]==1: #network.graph.nodes[i]['borderlink']==1:
                                        wall_id=i
                                    else:
                                        wall_id=wid1
                                    #Which of the 2 polygons associated to wall node "wall_id" do we add the twpid to?
                                    #Only one of them is related to junction node "j". Need to look at position into Wall2Junction
                                    for id1, val in enumerate(Wall2Junction[wall_id]): 
                                        if val==j: #Finding the position "id1" (index equal to 0 or 1) of the current junction in the list of junctions related to the wall "i" 
                                            break #2*i+id1 is the row of the polygon connecting wall "i" to junction "j"
                                    if ThickWallsX[int(ThickWallPolygonX[int(2*wall_id+id1)][1])][4]==cid: #The polygon nodes need to be consecutive ("no zigzag"), hence the ordering of the 3rd and 4th twpid matter
                                        ThickWallPolygonX[int(2*wall_id+id1)][3]=twpidX
                                    else:
                                        ThickWallPolygonX[int(2*wall_id+id1)][2]=twpidX
                                    nThickWallPolygonX[int(2*wall_id+id1)]+=1
                                    #New walls ID for each original wall
                                    Wall2NewWallX[j][int(nWall2NewWallX[j])]=twpidX
                                    nWall2NewWallX[j]+=1
                                    twpidX+=1
    ...
    if general.paraview==1 or general.par_track==1 or general.apo_contagion>0 or general.sym_contagion>0:
        ...
        for j in range(network.n_walls, network.n_wall_junction):
            for cid in Junction2Wall2Cell[j-network.n_walls]:
                if cid-network.n_wall_junction in hormones.apo_zombie0:
                    cc=hormones.apo_cc[hormones.apo_zombie0.index(cid-network.n_wall_junction)]
    ...
    for h in range(Nhydraulics):
        ...
        Flow_xyl=empty((n_xylem_cells+1,Nscenarios))
        Flow_xyl[:]=np.nan
        ...
        STFcell_plus=zeros((network.n_cells,Nmaturity))
        STFcell_minus=zeros((network.n_cells,Nmaturity))
        ...
        for Maturity in geometry.maturity_elems:
            ...
            if not isnan(Flow_xyl[0][count]):
                if isnan(Psi_xyl[iMaturity][count]) and isnan(dPsi_xyl[iMaturity][count]):
                    ...
                    for cid in network.xylem_cells:
                        area=cellarea[cid-network.n_wall_junction]
            ...
            if ratio_cortex==1: #Uniform AQP activity in all cortex membranes
                ...
                for cell_group in network.cellset['cell_to_wall']: #Loop on cells. network.cellset['cell_to_wall'] contains cell wall groups info (one group by cell)
                    ...
                    for r in cell_group: #Loop for wall elements around the cell
                        wall_id= int(r.get("id")) #Cell wall ID
                        if network.graph.nodes[network.n_wall_junction + cellnumber1]['cgroup']==4: #Cortex
                            dist_cell=sqrt(square(position[wall_id][0]-position[network.n_wall_junction+cellnumber1][0])+square(position[wall_id][1]-position[network.n_wall_junction+cellnumber1][1])) #distance between wall node and cell node (micrometers)
                            surf=(height+dist_cell)*network.wall_lengths[wall_id]*1.0E-08 #(square centimeters)
                            temp+=surf*1.0E-04*(dist_grav[wall_id]+(ratio_cortex*dmax_cortex-dmin_cortex)/(1-ratio_cortex))
            ...
            if general.apo_contagion==2 and general.sym_contagion==2:
                ...
                for i in range(network.n_walls):
                    if i in network.apo_wall_zombies0:
                        ...
                        matrix_C[i][i]-=hormones.degrad1*1.0E-12*(lat_dists[i][0]*geometry.thickness*network.wall_lengths[i]+height*geometry.thickness*network.wall_lengths[i]/2-square(geometry.thickness)*network.wall_lengths[i])
                for j in range(network.n_walls,network.n_wall_junction):
                ...
                for cellnumber1 in range(network.n_cells):
                    if cellnumber1 in hormones.sym_zombie0:
                        matrix_C[network.n_wall_junction+cellnumber1][network.n_wall_junction+cellnumber1]=1.0
                        rhs_C[network.n_wall_junction+cellnumber1][0]=hormones.sym_cc[hormones.sym_zombie0.index(cellnumber1)] #1.0 #Concentration in source protoplasts defined in geometry_config
                    else: #Decomposition rate (mol decomp/mol-day * cm^3)
                        matrix_C[network.n_wall_junction+cellnumber1][network.n_wall_junction+cellnumber1]-=hormones.degrad1*1.0E-12*cellarea[cellnumber1]*height
                ...
                for i in range(network.n_walls):
                    if i in network.apo_wall_zombies0:
                        ...
                        matrix_ApoC[i][i]-=hormones.degrad1*1.0E-12*(lat_dists[i][0]*geometry.thickness*network.wall_lengths[i]+height*geometry.thickness*network.wall_lengths[i]/2-square(geometry.thickness)*network.wall_lengths[i])
                for j in range(network.n_walls,network.n_wall_junction):
                    if j in Apo_j_Zombies0:
                        matrix_ApoC[j][j]=1.0
            ...
            if Barrier>0: #K_xyl_spec calculated from Poiseuille law (cm^3/hPa/d)
                for cid in network.xylem_cells:
                    K_axial[cid]=cellarea[cid-network.n_wall_junction]**2/(8*3.141592*height*1.0E-05/3600/24)*1.0E-12 #(micron^4/micron)->(cm^3) & (1.0E-3 Pa.s)->(1.0E-05/3600/24 hPa.d) 
                ...
                for neighboor, eattr in edges.items(): #Loop on connections (edges)
                    ...
                    if j > i: #Only treating the information one way to save time
                        ...
                        if path == 'wall': #Wall connection
                            ...
                            Kmb[jmb]=K
                            #if jmb<=10:
                            #    print(jmb,'K init',K,'wall_id',i,'cid',j-network.n_wall_junction)
                            jmb+=1
                            ...
                            if ((j-network.n_wall_junction in geometry.intercellular_ids) or (i-network.n_wall_junction in geometry.intercellular_ids)) and Barrier>0: #one of the connected cells is an intercellular space "cell".
                                ...
                                if (i-network.n_wall_junction in PPP) or (j-network.n_wall_junction in PPP):
            ...
            for wall_id in network.border_walls:
                if (position[wall_id][0]>=Xcontact) or (Wall2Cell[wall_id][0]-network.n_wall_junction in hormones.contact): #Wall (not including junctions) connected to soil
                    ...
                    #    rhs_C[wall_id][0] -= temp*Diff1*Os_soil[0][count]
                        
            #Adding matrix components at soil-junction connections
            for network.n_junctions in network.border_junction:
                if (position[network.n_junctions][0]>=Xcontact) or (Junction2Wall2Cell[network.n_junctions-network.n_walls][0]-network.n_wall_junction in hormones.contact) or (Junction2Wall2Cell[network.n_junctions-network.n_walls][1]-network.n_wall_junction in hormones.contact) or (Junction2Wall2Cell[network.n_junctions-network.n_walls][2]-network.n_wall_junction in hormones.contact): #Junction connected to soil
                    ...
                    #    rhs_C[network.n_junctions][0] -= temp*Diff1*Os_soil[0][count]
            ...
            if Barrier>0:
                if not isnan(Psi_xyl[iMaturity][count]): #Pressure xylem BC
                    for cid in network.xylem_cells:
                        ...
                        #if C_flag:
                        #    temp=10E-04*((cellperimeter[cid-network.n_wall_junction]/2)**2)/pi/height #Cell approximative cross-section area (cm^2) per length (cm)
            ...
            if Barrier>0:
                if not isnan(Psi_xyl[iMaturity][count]):
                    for cid in network.xylem_cells:
                        ...
                        rank=int(Cell_rank[cid-network.n_wall_junction][0])
            ...
            if Barrier>0 and isnan(Psi_xyl[iMaturity][0]):
                ...
                for cid in network.xylem_cells:
                    Psi_xyl[iMaturity][0]+=soln[cid][0]/n_xylem_cells #Average of xylem water pressures
                ...
                if i<network.n_walls: #wall ID
                    ...
                    for neighboor, eattr in edges.items(): #Loop on connections (edges)
                        ...
                        if path == "membrane": #Membrane connection
                            ...
                            K=Kmb[jmb][0]
                            #if jmb<=10:
                            #    print(jmb,'K STF',K,'wall_id',i,'cid',j-network.n_wall_junction)
                            jmb+=1
                            ...
                            rank = int(Cell_rank[j-network.n_wall_junction][0])
                            row = int(rank2row[rank][0])
            ...
            for count in range(1,Nscenarios):
                        ...
                        for neighboor, eattr in edges.items(): #Loop on connections (edges)
                            ...
                            if j > i: #Only treating the information one way to save time
                                ...
                                if path == "membrane": #Membrane connection
                                    ...
                                    rank=int(Cell_rank[int(j-network.n_wall_junction)])
                                    row=int(rank2row[rank][0])
                                    if rank==1:#Exodermis
                ...
                if Barrier==0: #No elongation from the Casparian strip on
                    for wall_id in range(network.n_walls):
                        rhs_e[wall_id][0]=network.wall_lengths[wall_id]*geometry.thickness/2*1.0E-08*(Elong_cell[0][count]+(x_rel[wall_id]-0.5)*Elong_cell_side_diff[0][count])*Water_fraction_apo #cm^3/d Cell wall horizontal surface assumed to be rectangular (junctions are pointwise elements)
                    for cid in range(network.n_cells):
                        if cellarea[cid]>cellperimeter[cid]*geometry.thickness/2:
                            rhs_e[network.n_wall_junction+cid][0]=(cellarea[cid]-cellperimeter[cid]*geometry.thickness/2)*1.0E-8*(Elong_cell[0][count]+(x_rel[network.n_wall_junction+cid]-0.5)*Elong_cell_side_diff[0][count])*Water_fraction_sym #cm^3/d Wall geometry.thickness removed from cell horizontal area to obtain protoplast horizontal area
                        else:
                ...
                for ind in range(network.n_wall_junction,len(network.graph.nodes)): #network.n_wall_junction is the index of the first cell
                    cellnumber1=ind-network.n_wall_junction
                ...
                if Barrier>0 and isnan(Psi_xyl[iMaturity][count]):
                    ...
                    for cid in network.xylem_cells:
                        Psi_xyl[iMaturity][count]+=soln[cid][0]/n_xylem_cells
                if Barrier>0:
                    if isnan(Psi_sieve[iMaturity][count]):
                            ...
                            if general.paraview==1 or general.par_track==1 or general.apo_contagion>0 or general.sym_contagion>0:
                                if path == "wall":
                                    ...
                                    if network.graph.nodes[j]['cgroup']==1: #Exodermis
                                        ...
                                        kaqp=float(a_cortex*dist_grav[wall_id]*1.0E-04+b_cortex) #AQP activity (cm/hPa/d)
                                        if kaqp < 0:
                                            error('Error, negative kaqp in cortical cell, adjust Paqp_cortex')
                                    ...
                                    #Macroscopic distributed parameter for transmembrane flow
                                    #Discretization based on cell layers and apoplasmic barriers
                                    rank = int(Cell_rank[j-network.n_wall_junction])
                                    row = int(rank2row[rank][0])
                                    if rank == 1 and count_epi > 0: #Outer exodermis
                                        row += 1
                                    if rank == 3 and count_cortex > 0: #Outer endodermis
                                    ...
                                    if psi_o_cell == inf:
                                        ...
                                        if general.sym_contagion==1:
                                            itemp=0
                                            while not Cell_connec[ind_o_cell-network.n_wall_junction][itemp] == j-network.n_wall_junction:
                                                itemp+=1
                                            Cell_connec_flow[ind_o_cell-network.n_wall_junction][itemp]=sign(temp)
                                            itemp=0
                                            while not Cell_connec[j-network.n_wall_junction][itemp] == ind_o_cell-network.n_wall_junction:
                                                itemp+=1
                ...
                if general.apo_contagion==2 or general.sym_contagion==2: #Sym & Apo contagion
                    ...
                    #for wall_id in network.xylem_walls:
                    #    temp=1.0E-04*(network.wall_lengths[wall_id]*height)/geometry.thickness #Section to length ratio (cm) for the xylem wall
                    #    if not temp==0:
                    #        list_walls_apo_diff.append(wall_id)
                    #    matrix_C[wall_id][wall_id] -= temp*Diff1 #Adding BC diffusion term
                    #    rhs_C[wall_id][0] -= temp*Diff1*Os_xyl[0][count] #new #Adding BC diffusion term
                    #for wall_id in network.border_walls:
                    #    if (position[wall_id][0]>=Xcontact) or (Wall2Cell[wall_id][0]-network.n_wall_junction in hormones.contact): #Wall (not including junctions) connected to soil
                    #        temp=1.0E-04*(network.wall_lengths[wall_id]/2*height)/(geometry.thickness/2)
                    #        if not temp==0:
                    #            list_walls_apo_diff.append(wall_id)
                    #        matrix_C[wall_id][wall_id] -= temp*Diff1 #Adding diffusion BC at soil junction
                    #        rhs_C[wall_id][0] -= temp*Diff1*Os_soil[0][count] #Adding BC diffusion term
                    #for network.n_junctions in network.border_junction:
                    #    if (position[network.n_junctions][0]>=Xcontact) or (Junction2Wall2Cell[network.n_junctions-network.n_walls][0]-network.n_wall_junction in hormones.contact) or (Junction2Wall2Cell[network.n_junctions-network.n_walls][1]-network.n_wall_junction in hormones.contact) or (Junction2Wall2Cell[network.n_junctions-network.n_walls][2]-network.n_wall_junction in hormones.contact): #Junction connected to soil
                    #        temp=1.0E-04*(network.wall_lengths[network.n_junctions]*height)/(geometry.thickness/2)
                    #        if not temp==0:
                    #            list_walls_apo_diff.append(network.n_junctions)
                    #        matrix_C[network.n_junctions][network.n_junctions] -= temp*Diff1 #Adding diffusion BC at soil junction
                    #        rhs_C[network.n_junctions][0] -= temp*Diff1*Os_soil[0][count] #Adding BC diffusion term
                    #
                    #Nwalls_apo_diff=np.zeros((network.n_wall_junction,2))
                    #Nwalls_apo_conv=np.zeros((network.n_wall_junction,2))
                    #Nwalls_TM_conv=np.zeros((network.n_wall_junction,2))
                    #for wall_id in list_walls_apo_diff:
                    #    Nwalls_apo_diff[wall_id][1]+=1
                    #    Nwalls_apo_diff[wall_id][0]=wall_id
                    #for wall_id in list_walls_apo_conv:
                    #    Nwalls_apo_conv[wall_id][1]+=1
                    #    Nwalls_apo_conv[wall_id][0]=wall_id
                    #for wall_id in list_walls_TM_conv:
                    #    Nwalls_TM_conv[wall_id][1]+=1
                    #    Nwalls_TM_conv[wall_id][0]=wall_id
                    #
                ...
                if general.apo_contagion==2:
                    ...
                    #    if (position[network.n_junctions][0]>=Xcontact) or (Junction2Wall2Cell[network.n_junctions-network.n_walls][0]-network.n_wall_junction in hormones.contact) or (Junction2Wall2Cell[network.n_junctions-network.n_walls][1]-network.n_wall_junction in hormones.contact) or (Junction2Wall2Cell[network.n_junctions-network.n_walls][2]-network.n_wall_junction in hormones.contact): #Junction connected to soil
                    #        temp=1.0E-04*(network.wall_lengths[network.n_junctions]*height)/(geometry.thickness/2)
                    #        matrix_C[network.n_junctions][network.n_junctions] += temp*Diff1 #Removing diffusion BC at soil junction
                    #        rhs_C[network.n_junctions][0] += temp*Diff1*Os_soil[0][count] #Removing BC diffusion term
                    #for wall_id in network.border_walls:
                    #    if (position[wall_id][0]>=Xcontact) or (Wall2Cell[wall_id][0]-network.n_wall_junction in hormones.contact): #Wall (not including junctions) connected to soil
                    #        temp=1.0E-04*(network.wall_lengths[wall_id]/2*height)/(geometry.thickness/2)
                    #        matrix_C[wall_id][wall_id] += temp*Diff1 #Removing diffusion BC at soil junction
                    #        rhs_C[wall_id][0] += temp*Diff1*Os_soil[0][count] #Removing BC diffusion term
                    #for wall_id in network.xylem_walls:
                    #    temp=1.0E-04*(network.wall_lengths[wall_id]*height)/geometry.thickness #Section to length ratio (cm) for the xylem wall
                    #    matrix_C[wall_id][wall_id] += temp*Diff1 #Removing BC diffusion term
                    #    rhs_C[wall_id][0] += temp*Diff1*Os_xyl[0][count] #new #Removing BC diffusion term
                ...
                if general.sym_contagion==1:
                    ...
                    with open(newpath+"Sym_Contagion_bottomb"+str(Barrier)+","+str(iMaturity)+"s"+str(count)+".pvtk", "a") as myfile:
                        ...
                        for ThickWallNode in ThickWalls:
                            cellnumber1=ThickWallNode[2]-network.n_wall_junction
                    ...
                    with open(newpath+"Sym_Contagion_bottomb"+str(Barrier)+","+str(iMaturity)+"s"+str(count)+".pvtk", "a") as myfile:
                        ...
                        if general.apo_contagion==2:
                            for ThickWallNode in ThickWalls:
                                cellnumber1=ThickWallNode[2]-network.n_wall_junction
                                #print(cellnumber1, soln_C[int(cellnumber1)+network.n_wall_junction])
                                myfile.write(str(float(soln_C[int(cellnumber1+network.n_wall_junction)])) + " \n")
                        else:
                            for ThickWallNode in ThickWalls:
                                cellnumber1=ThickWallNode[2]-network.n_wall_junction
                    ...
                    ##sath01=max(soln[network.n_wall_junction:network.n_wall_junction+network.n_cells-1])
                    ##satl01=min(soln[network.n_wall_junction:network.n_wall_junction+network.n_cells-1])
                    ...
                    #    myfile.write("LOOKUP_TABLE default \n")
                    #    for node in network.graph:
                    #        if node-network.n_wall_junction in [Zombie0]: #Source cell
                    #            myfile.write(str(float(0.0)) + " \n")
                    #        elif node-network.n_wall_junction in Zombies:
                ...
                if general.apo_contagion==1:
                    ...
                    with open(newpath+"Apo_Contagion_bottomb"+str(Barrier)+","+str(iMaturity)+"s"+str(count)+".pvtk", "a") as myfile:
                        ...
                        myfile.write(" \n")
                        myfile.write("CELLS " + str(int(network.n_wall_junction+network.n_walls-len(list_ghostwalls)*2-len(list_ghostjunctions))) + " " + str(int(2*network.n_walls*5-len(list_ghostwalls)*10+sum(nWall2NewWallX[network.n_walls:])+network.n_wall_junction-network.n_walls+2*len(Wall2NewWallX[network.n_walls:])-nGhostJunction2Wall-len(list_ghostjunctions))) + " \n") #The number of cells corresponds to the number of lines in ThickWalls (if no ghost wall & junction)
                        i=0
