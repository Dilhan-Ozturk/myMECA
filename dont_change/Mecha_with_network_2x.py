...
def mecha(General='./extdata/Maize_General.xml',#'Arabido1_General.xml' #'MilletLR3_General.xml' #
    ...
    print('[1/x] Importing General data')
    
    general = GeneralData(General)

    OS=etree.parse(General).getroot().xpath('OS')[0].get("value")
    Output_path=etree.parse(General).getroot().xpath('Output')[0].get("path")
    Paraview=int(etree.parse(General).getroot().xpath('Paraview')[0].get("value"))
    ParaviewWF=int(etree.parse(General).getroot().xpath('Paraview')[0].get("WallFlux"))
    ParaviewMF=int(etree.parse(General).getroot().xpath('Paraview')[0].get("MembraneFlux"))
    ParaviewPF=int(etree.parse(General).getroot().xpath('Paraview')[0].get("PlasmodesmataFlux"))
    ParaviewWP=int(etree.parse(General).getroot().xpath('Paraview')[0].get("WallPot"))
    ParaviewCP=int(etree.parse(General).getroot().xpath('Paraview')[0].get("CellPot"))
    ParTrack=int(etree.parse(General).getroot().xpath('ParTrack')[0].get("value"))
    Sym_Contagion=int(etree.parse(General).getroot().xpath('Sym_Contagion')[0].get("value"))
    Apo_Contagion=int(etree.parse(General).getroot().xpath('Apo_Contagion')[0].get("value"))
    color_threshold=float(etree.parse(General).getroot().xpath('color_threshold')[0].get("value"))
    thickness_disp=float(etree.parse(General).getroot().xpath('thickness_disp')[0].get("value"))
    thicknessJunction_disp=float(etree.parse(General).getroot().xpath('thicknessJunction_disp')[0].get("value"))
    radiusPlasmodesm_disp=float(etree.parse(General).getroot().xpath('radiusPlasmodesm_disp')[0].get("value"))
                    ...
                    if G.nodes[j]['type']=='apo': #then j is a junction node 
                        for cid in Wall2Cell[i]: #Wall2Cell[i][k] are the cell node ID associated to Wall i
                            if cid not in Junction2Wall2Cell[j-Nwalls]: 
                                Junction2Wall2Cell[j-Nwalls][int(nJunction2Wall2Cell[j-Nwalls][0])]=cid #Writes the cells indirectly associated to each junction
            ...
            if Barrier>0:
                if not isnan(Psi_xyl[iMaturity][count]):
                    for cid in listxyl:
                        ...
                        rank=int(Cell_rank[cid-NwallsJun][0])
                ...
                if i<Nwalls: #wall ID
                    ...
                    for neighboor, eattr in edges.items(): #Loop on connections (edges)
                        ...
                        if path == "membrane": #Membrane connection
                            ...
                            rank = int(Cell_rank[j-NwallsJun][0])
                            row = int(rank2row[rank][0])
