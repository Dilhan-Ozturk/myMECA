...
def mecha(Gen='./extdata/Maize_General.xml',#'Arabido1_General.xml' #'MilletLR3_General.xml' #
    ...
    # good
    #Import cellset data
    tree = etree.parse(cellsetdata) #Maize_Charles\\Maize_pip_cross4.xml') # #Parse literally decrypts the tree element data         SteleOK_high.xml
    ...
    for p in points: #Loop on wall elements (groups of points)
        ...
        for r in p: #Loop on points within the wall element to calculate their average X and Y coordinates 
            x= config.im_scale*float(r.get("x")) #X coordinate of the point
            y= config.im_scale*float(r.get("y")) #Y coordinate of the point
    ...
    for w in Cell2Wall_loop: #Loop on cells. Cell2Wall_loop contains cell wall groups info (one group by cell)
        ...
        if config.apo_contagion:
            if cellnumber1 in config.apo_zombie0:
                cc=config.apo_cc[config.apo_zombie0.index(cellnumber1)]
                for r in w: #w points to the cell walls around the current cell
                    wid= int(r.get("id")) #Wall ID number
                    if wid not in Apo_w_Zombies0:
                        Apo_w_Zombies0.append(wid)
                        Apo_w_cc.append(cc)
            if cellnumber1 in config.apo_target:
                for r in w: #w points to the cell walls around the current cell
                    wid= int(r.get("id")) #Wall ID number
                    if wid not in Apo_w_Target:
                        Apo_w_Target.append(wid)
            if cellnumber1 in config.apo_immune:
                for r in w: #w points to the cell walls around the current cell
                    wid= int(r.get("id")) #Wall ID number
    ...
    for w in Cell2Wall_loop: #Loop on cells. Cell2Wall_loop contains cell wall groups info (one group by cell)
        ...
        if celltype==4: #Cortex
            if any(Cell_rank[Cell_connec[cellnumber1][0:nCell_connec[cellnumber1][0]]]==3): #Cell to cell connection with endodermis
                ...
                if cellperimeter[cellnumber1]<config.interc_perims[0]:
                    config.intercellular_ids.append(cellnumber1) #Cell id starting at 0
                ...
                if cellperimeter[cellnumber1]<config.interc_perims[4]:
                    config.intercellular_ids.append(cellnumber1) #Cell id starting at 0
        ...
        for w in Cell2Wall_loop: #Loop on cells. Cell2Wall_loop contains cell wall groups info (one group by cell)
           ...
           # if i<4: #Within 3 layers of cortical sides
                if any(Cell_rank[Cell_connec[cellnumber1][0:nCell_connec[cellnumber1][0]]]==(40+i)): #Cell to cell connection with endodermis
                    ...
                    if i==0 and cellperimeter[cellnumber1]<config.interc_perims[1]:
                        config.intercellular_ids.append(cellnumber1)
                    elif i==1 and cellperimeter[cellnumber1]<config.interc_perims[2]:
                        config.intercellular_ids.append(cellnumber1)
                    elif i==2 and cellperimeter[cellnumber1]<config.interc_perims[3]:
                        config.intercellular_ids.append(cellnumber1)
                    elif i>2 and cellperimeter[cellnumber1]<config.interc_perims[4]:
                        config.intercellular_ids.append(cellnumber1)
    ...
    config.intercellular_ids=config.intercellular_ids[1:]
    ...
    for node, edges in G.adjacency() :
        ...
        if i>=NwallsJun: #Cell
            if G.nodes[i]['cgroup']==16 or G.nodes[i]['cgroup']==21:
                ...
                if i-NwallsJun not in config.intercellular_ids: #The loop focuses on exo, cortex and endodermis cells that are not intercellular spaces
                    for neighboor, eattr in edges.items(): #Loop on connections (edges)
                        if eattr['path'] == "plasmodesmata": #Plasmodesmata connection  #eattr is the edge attribute (i.e. connection type)
                            ...
                            if (G.nodes[i]['cgroup']==outercortex_connec_rank and G.nodes[j]['cgroup']==4) or (G.nodes[j]['cgroup']==outercortex_connec_rank and G.nodes[i]['cgroup']==4):#Exodermis to cortex cell or vice versa (epidermis if no exodermis exists)
                                ...
                                if j-NwallsJun not in config.intercellular_ids:
    ...
    if config.paraview==1 or config.par_track==1 or config.apo_contagion>0 or config.sym_contagion>0:
                ...
                for neighboor, eattr in edges.items(): #Loop on connections (edges)
                    ...
                    if G.nodes[cid]['type']=='cell':
                        if not position[cid][0]==position[wid][0]:
                            ...
                            slopeCG=inf
                        x=position[wid][0]+cos(arctan(slopeCG))*config.thickness_disp/2*sign(position[cid][0]-position[wid][0]) #-> new position of the wall node on the current cell side
                        y=position[wid][1]+sin(arctan(slopeCG))*config.thickness_disp/2*sign(position[cid][0]-position[wid][0])
                        ...
                        if Borderlink[wid]==1: #G.nodes[wid]['borderlink']==1:
                            x=position[wid][0]-cos(arctan(slopeCG))*config.thickness_disp/2*sign(position[cid][0]-position[wid][0]) #-> new position of the wall node opposite to the current cell side
                            y=position[wid][1]-sin(arctan(slopeCG))*config.thickness_disp/2*sign(position[cid][0]-position[wid][0])
        ...
        x_rel[:]=np.nan
        L_diff = (
            (abs(float(Layer_dist[3][0] - Layer_dist[2][0]) * 1.0E-04)),  # Diffusion length (cm)
            (abs(float(Layer_dist[3][0] - xyl80_dist) * 1.0E-04))          # Diffusion length (cm)
        )
        for node, edges in G.adjacency():
            wid=indice[node]
            if wid<Nwalls: #wall that is not a junction (connected to a cell)
                ...
                cid2=Wall2Cell[wid][1]
                rank1=Cell_rank[int(cid1-NwallsJun)]
                row1=rank2row[int(rank1[0])].item()
                if not isnan(cid2):
                    rank2=Cell_rank[int(cid2-NwallsJun)]
                    row2 = rank2row[int(rank2[0])].item()
                    ...
                    if G.nodes[j]['type']=='apo': #then j is a junction node 
                        for cid in Wall2Cell[i][0:int(nWall2Cell[i])]: #Wall2Cell[i][k] are the cell node ID associated to Wall i
                            if cid not in Junction2Wall2Cell[j-Nwalls]: 
                                ...
                                x=position[j][0]+cos(arctan(slopeCG))*config.thickness_junction_disp/2*sign(position[cid][0]-position[j][0])
                                y=position[j][1]+sin(arctan(slopeCG))*config.thickness_junction_disp/2*sign(position[cid][0]-position[j][0])
                                ...
                                if Borderlink[j]==1: #G.nodes[j]['borderlink']==1: #If the wall is at a border, there is only one cell => need to add the opposite new wall node independently
                                    if not slopeCG==0.0:
                                        ...
                                        slopeCG=inf
                                    x=position[j][0]-cos(arctan(slopeCG))*config.thickness_disp/2*sign(position[cid][0]-position[j][0]) #-> new position of the wall node opposite to the current cell side
                                    y=position[j][1]-sin(arctan(slopeCG))*config.thickness_disp/2*sign(position[cid][0]-position[j][0])
        ...
        for node, edges in G.adjacency():
            ...
            if i<Nwalls: #wall that is not a junction (connected to a cell)
                for neighboor, eattr in edges.items(): #Loop on connections (edges) separated to make sure that Wall2Cell[i] is complete
                    j=indice[neighboor]
                    if G.nodes[j]['type']=='apo': #then j is a junction node 
                        for cid in Wall2Cell[i][0:int(nWall2Cell[i].item())]: #Wall2Cell[i][k] are the cell node ID associated to Wall i
                            if cid not in Junction2Wall2Cell[j-Nwalls]: 
                                Junction2Wall2Cell[j-Nwalls][int(nJunction2Wall2Cell[j-Nwalls].item())] = cid #Writes the cells indirectly associated to each junction
                                nJunction2Wall2Cell[j-Nwalls]+=1
    ...
    if config.paraview==1 or config.par_track==1 or config.apo_contagion>0 or config.sym_contagion>0:
        ...
        for j in range(Nwalls, NwallsJun):
            for cid in Junction2Wall2Cell[j-Nwalls]:
                if cid-NwallsJun in config.apo_zombie0:
                    cc=config.apo_cc[config.apo_zombie0.index(cid-NwallsJun)]
    ...
    Nmaturity=len(config.maturity_stages)
    ...
    for h in range(Nhydraulics):
        ...
        UptakeLayer_plus=zeros((int(r_discret[0].item()),Nmaturity,Nscenarios))
        UptakeLayer_minus=zeros((int(r_discret[0].item()),Nmaturity,Nscenarios))
        Q_xyl_layer=zeros((int(r_discret[0].item()),Nmaturity,Nscenarios))
        Q_sieve_layer=zeros((int(r_discret[0].item()),Nmaturity,Nscenarios))
        Q_elong_layer=zeros((int(r_discret[0].item()),Nmaturity,Nscenarios))
        STFmb=zeros((Nmb,Nmaturity))
        STFcell_plus=zeros((Ncells,Nmaturity))
        STFcell_minus=zeros((Ncells,Nmaturity))
        STFlayer_plus=zeros((int(r_discret[0].item()),Nmaturity))
        STFlayer_minus=zeros((int(r_discret[0].item()),Nmaturity))
        PsiCellLayer=zeros((int(r_discret[0].item()),Nmaturity,Nscenarios))
        PsiWallLayer=zeros((int(r_discret[0].item()),Nmaturity,Nscenarios))
        OsCellLayer=zeros((int(r_discret[0].item()),Nmaturity,Nscenarios))
        nOsCellLayer=zeros((int(r_discret[0].item()),Nmaturity,Nscenarios))
        OsWallLayer=zeros((int(r_discret[0].item()),Nmaturity,Nscenarios))
        nOsWallLayer=zeros((int(r_discret[0].item()),Nmaturity,Nscenarios)) #Used for averaging OsWallLayer
        NWallLayer=zeros((int(r_discret[0].item()),Nmaturity,Nscenarios))
        ...
        for Maturity in Maturityrange:
            ...
            if config.apo_contagion==2 and config.sym_contagion==2:
                ...
                for i in range(Nwalls):
                    if i in Apo_w_Zombies0:
                        ...
                        matrix_C[i][i]-=config.degrad1*1.0E-12*(lat_dists[i][0]*config.thickness*lengths[i]+height*config.thickness*lengths[i]/2-square(config.thickness)*lengths[i])
                for j in range(Nwalls,NwallsJun):
                    if j in Apo_j_Zombies0:
                        matrix_C[j][j]=1.0
                        rhs_C[j][0]=Apo_j_cc[Apo_j_Zombies0.index(j)] #1.0 #Concentration in source junction j defined in Geom
                    else: #Decomposition rate (mol decomp/mol-day * cm^3)
                        matrix_C[j][j]-=config.degrad1*1.0E-12*height*config.thickness*lengths[j]/2
                for cellnumber1 in range(Ncells):
                    if cellnumber1 in config.sym_zombie0:
                        ...
                        rhs_C[NwallsJun+cellnumber1][0]=config.sym_cc[config.sym_zombie0.index(cellnumber1)] #1.0 #Concentration in source protoplasts defined in Geom
                    else: #Decomposition rate (mol decomp/mol-day * cm^3)
                        matrix_C[NwallsJun+cellnumber1][NwallsJun+cellnumber1]-=config.degrad1*1.0E-12*cellarea[cellnumber1]*height
                ...
                for i in range(Nwalls):
                    if i in Apo_w_Zombies0:
                        ...
                        matrix_ApoC[i][i]-=config.degrad1*1.0E-12*(lat_dists[i][0]*config.thickness*lengths[i]+height*config.thickness*lengths[i]/2-square(config.thickness)*lengths[i])
                for j in range(Nwalls,NwallsJun):
                    if j in Apo_j_Zombies0:
                        matrix_ApoC[j][j]=1.0
                        rhs_ApoC[j][0]=Apo_j_cc[Apo_j_Zombies0.index(j)] #1 #Concentration in source junction j equals 1 by default
                    else: #Decomposition rate (mol decomp/mol-day * cm^3)
                        matrix_ApoC[j][j]-=config.degrad1*1.0E-12*height*config.thickness*lengths[j]/2
                ...
                for cellnumber1 in range(Ncells):
                    if cellnumber1 in config.sym_zombie0:
                        ...
                        matrix_SymC[cellnumber1][cellnumber1]-=config.degrad1*1.0E-12*cellarea[cellnumber1]*height
                    ...
                    for neighboor, eattr in edges.items(): #Loop on connections (edges)
                        if eattr['path'] == 'membrane': #Wall connection
                            if any(passage_cell_ID==array((indice[neighboor])-NwallsJun)):
                                count_passage+=1
                            if any(config.intercellular_ids==array((indice[neighboor])-NwallsJun)):
                                count_interC+=1
                        ...
                        if path == 'wall': #Wall connection
                            ...
                            if config.apo_contagion==2 and config.sym_contagion==2:
                                for carrier in config.carrier_elems:
                                    if int(carrier.get("tissue"))==G.nodes[j]['cgroup']:
                                        ...
                                        if j-NwallsJun not in config.intercellular_ids and not (Barrier>0 and (G.nodes[j]['cgroup']==13 or G.nodes[j]['cgroup']==19 or G.nodes[j]['cgroup']==20)):
                                            ...
                                            if int(carrier.get("direction"))==1: #Influx transporter
                                                if j-NwallsJun not in config.sym_zombie0: #Concentration not affected if set as boundary condition
                            ...
                            if G.nodes[j]['cgroup']==1: #Exodermis
                                ...
                                if Barrier>0: #Xylem vessel
                                    ...
                                    if config.apo_contagion==2 and config.sym_contagion==2:
                                        ...
                                        temp=1.0E-04*(lengths[i]*height)/config.thickness #Section to length ratio (cm) for the xylem wall
                                        if i not in Apo_w_Zombies0:
                                            matrix_C[i][i] -= temp*config.diff_pw1
                                            matrix_C[i][j] += temp*config.diff_pw1
                                        if j-NwallsJun not in config.sym_zombie0: #Mature xylem vessels are referred to as cells, so they are on the Sym side even though they are part of the apoplast
                                ...
                                kaqp=kaqp_stele
                            elif (j-NwallsJun in config.intercellular_ids) and Barrier>0: #the neighbour is an intercellular space "cell". Between j and i connected by a membrane, only j can be cell because j>i
                                kaqp=config.k_interc
                                #No carrier
                            elif G.nodes[j]['cgroup']==4: #Cortex
                                kaqp=float(a_cortex*dist_grav[i].item()*1.0E-04+b_cortex) #AQP activity (cm/hPa/d)
                                if kaqp < 0:
                            ...
                            if count_endo>=2: #wall between two endodermis cells, in this case the suberized wall can limit the transfer of water between cell and wall
                                if kw_endo_endo==0.00:
                                    ...
                                    K = 1/(1/(kw_endo_endo/(config.thickness/2*1.0E-04))+1/(kmb+kaqp))*1.0E-08*(height+eattr['dist'])*eattr['length']
                                ...
                                if kw_exo_exo==0.00:
                                    ...
                                    K = 1/(1/(kw_exo_exo/(config.thickness/2*1.0E-04))+1/(kmb+kaqp))*1.0E-08*(height+eattr['dist'])*eattr['length']
                                ...
                                if count_passage>0:
                                    K = 1/(1/(kw_passage/(config.thickness/2*1.0E-04))+1/(kmb+kaqp))*1.0E-08*(height+eattr['dist'])*eattr['length']
                                else:
                                    if kw_endo_peri==0.00:
                                        K=0.00
                                    else:
                                        K = 1/(1/(kw_endo_peri/(config.thickness/2*1.0E-04))+1/(kmb+kaqp))*1.0E-08*(height+eattr['dist'])*eattr['length']
                                ...
                                if kaqp==0.0:
                                    ...
                                    if count_passage>0:
                                        ...
                                        if kw_endo_cortex==0.00:
                                            ...
                                            K = 1/(1/(kw_endo_cortex/(config.thickness/2*1.0E-04))+1/(kmb+kaqp))*1.0E-08*(height+eattr['dist'])*eattr['length']
                            else:
                                if kaqp==0.0:
                                    K=1.00E-16
                                else:
                                    K = 1/(1/(kw/(config.thickness/2*1.0E-04))+1/(kmb+kaqp))*1.0E-08*(height+eattr['dist'])*eattr['length']
                            Kmb[jmb]=K
                            #if jmb<=10:
                            ...
                            if ((j-NwallsJun in config.intercellular_ids) or (i-NwallsJun in config.intercellular_ids)) and Barrier>0: #one of the connected cells is an intercellular space "cell".
                                temp_factor=0.0
                            ...
                            if config.sym_contagion==2:
                                ...
                                if config.apo_contagion==2: #Sym & Apo contagion
                                    if i-NwallsJun not in config.sym_zombie0:
                                        matrix_C[i][i] -= DF
                                        matrix_C[i][j] += DF #Convection will be dealt with further down
                                    if j-NwallsJun not in config.sym_zombie0:
                                        matrix_C[j][j] -= DF
                                        matrix_C[j][i] += DF
            ...
            for wid in Borderwall:
                if (position[wid][0]>=Xcontact) or (Wall2Cell[wid][0]-NwallsJun in config.contact): #Wall (not including junctions) connected to soil
                    temp=1.0E-04*(lengths[wid]/2*height)/(config.thickness/2)
            ...
            for jid in Borderjunction:
                if (position[jid][0]>=Xcontact) or (Junction2Wall2Cell[jid-Nwalls][0]-NwallsJun in config.contact) or (Junction2Wall2Cell[jid-Nwalls][1]-NwallsJun in config.contact) or (Junction2Wall2Cell[jid-Nwalls][2]-NwallsJun in config.contact): #Junction connected to soil
                    temp=1.0E-04*(lengths[jid]*height)/(config.thickness/2)
                    K=kw*temp
            ...
            if Barrier>0:
                if not isnan(Psi_xyl[iMaturity][count]):
                    for cid in listxyl:
                        ...
                        rank=int(Cell_rank[cid-NwallsJun].item())
                        row=int(rank2row[rank].item())
                        Q_xyl_layer[row][iMaturity][count] += Q.item()
                elif not isnan(Flow_xyl[0][count]):
                    for cid in listxyl:
                        Q=-rhs_x[cid]
                        Q_xyl.append(Q) #(cm^3/d) Negative for water flowing into xylem tubes
                ...
                if not isnan(Psi_sieve[iMaturity][count]):
                    for cid in listprotosieve:
                        ...
                        rank=int(Cell_rank[cid-NwallsJun])
                        row=int(rank2row[rank])
                        Q_sieve_layer[row][iMaturity][count] += Q.item()
                elif not isnan(Flow_sieve[0][count]):
                    for cid in listprotosieve:
                        Q=-rhs_p[cid]
            ...
            if Barrier>0:
                if not isnan(Psi_xyl[iMaturity][0]):
                    kr_tot[iMaturity][0]=Q_tot[iMaturity][0].item()/(Psi_soil[0][0].item()-Psi_xyl[iMaturity][0].item())/perimeter.item()/height/1.0E-04
                else:
                ...
                if not isnan(Psi_sieve[iMaturity][0]):
                    kr_tot[iMaturity][0]=Q_tot[iMaturity][0].item()/(Psi_soil[0][0].item()-Psi_sieve[iMaturity][0].item())/perimeter.item()/height/1.0E-04
                else:
                ...
                if i<Nwalls: #wall ID
                    ...
                    for neighboor, eattr in edges.items(): #Loop on connections (edges)
                        ...
                        if path == "membrane": #Membrane connection
                            ...
                            rank = int(Cell_rank[j-NwallsJun].item())
                            row = int(rank2row[rank].item())
                            ...
                            if ((j-NwallsJun not in config.intercellular_ids) and (j not in listxyl)) or Barrier==0: #Not part of STF if crosses an intercellular space "membrane" or mature xylem "membrane" (that is no membrane though still labelled like one)
                                if Flow > 0 :
                                    UptakeLayer_plus[row][iMaturity][count] += Flow.item() #grouping membrane flow rates in cell layers
                                else:
                                    UptakeLayer_minus[row][iMaturity][count] += Flow.item()
                                if Flow/Q_tot[iMaturity][0] > 0 :
                                    ...
                                    STFcell_plus[j-NwallsJun][iMaturity] += Flow.item()/Q_tot[iMaturity][0].item() #Cell standard transmembrane fraction (positive)
                                    ...
                                    STFcell_minus[j-NwallsJun][iMaturity] += Flow.item()/Q_tot[iMaturity][0]#Cell standard transmembrane fraction (negative)
                                    #STFmb[jmb-1][iMaturity] = Flow/Q_tot[iMaturity][0]
            ...
            for count in range(1,Nscenarios):
                            ...
                            if j > i: #Only treating the information one way to save time
                                ...
                                if path == "membrane": #Membrane connection
                                    ...
                                    if rank==1:#Exodermis
                                        ...
                                        if j-NwallsJun in config.intercellular_ids:
                ...
                if Barrier==0: #No elongation from the Casparian strip on
                    for wid in range(Nwalls):
                        rhs_e[wid][0]=lengths[wid]*config.thickness/2*1.0E-08*(Elong_cell[0][count]+(x_rel[wid]-0.5)*Elong_cell_side_diff[0][count])*Water_fraction_apo #cm^3/d Cell wall horizontal surface assumed to be rectangular (junctions are pointwise elements)
                    for cid in range(Ncells):
                        if cellarea[cid]>cellperimeter[cid]*config.thickness/2:
                            rhs_e[NwallsJun+cid][0]=(cellarea[cid]-cellperimeter[cid]*config.thickness/2)*1.0E-8*(Elong_cell[0][count]+(x_rel[NwallsJun+cid]-0.5)*Elong_cell_side_diff[0][count])*Water_fraction_sym #cm^3/d Wall thickness removed from cell horizontal area to obtain protoplast horizontal area
                        else:
                            ...
                            if config.paraview==1 or config.par_track==1 or config.apo_contagion>0 or config.sym_contagion>0:
                                if path == "wall":
                                    ...
                                    if G.nodes[j]['cgroup']==1: #Exodermis
                                        ...
                                        kaqp=kaqp_stele
                                    elif (j-NwallsJun in config.intercellular_ids) and Barrier>0: #the neighbour is an intercellular space "cell"
                                        kaqp=config.k_interc
                                        noPD=True
                                    elif G.nodes[j]['cgroup']==4: #Cortex
                                    ...
                                    if count_endo>=2: #wall between two endodermis cells, in this case the suberized wall can limit the transfer of water between cell and wall
                                        if kw_endo_endo==0.00:
                                            ...
                                            K = 1/(1/(kw_endo_endo/(config.thickness/2*1.0E-04))+1/(kmb+kaqp))*1.0E-08*(height+eattr['dist'])*eattr['length'] #(cm^3/hPa/d)
                                    elif count_exo>=2: #wall between two exodermis cells, in this case the suberized wall can limit the transfer of water between cell and wall
                                        if kw_exo_exo==0.00:
                                            K=0.00
                                        else:
                                            K = 1/(1/(kw_exo_exo/(config.thickness/2*1.0E-04))+1/(kmb+kaqp))*1.0E-08*(height+eattr['dist'])*eattr['length'] #(cm^3/hPa/d)
                                    elif count_stele_overall>0 and count_endo>0: #wall between endodermis and pericycle, in this case the suberized wall can limit the transfer of water between cell and wall
                                        if count_passage>0:
                                            K = 1/(1/(kw_passage/(config.thickness/2*1.0E-04))+1/(kmb+kaqp))*1.0E-08*(height+eattr['dist'])*eattr['length']
                                        else:
                                        ...
                                        if kaqp==0.0:
                                            ...
                                            K = 1/(1/(kw/(config.thickness/2*1.0E-04))+1/(kmb+kaqp))*1.0E-08*(height+eattr['dist'])*eattr['length'] #(cm^3/hPa/d)
                                    ...
                                    jmb+=1
                                    if ((j-NwallsJun not in config.intercellular_ids) and (j not in listxyl)) or Barrier==0: #No aerenchyma in the elongation zone
                                        if Flow > 0 :
                                            ...
                                            if config.apo_contagion==2: #Apo & Sym Contagion
                                                if Fcc>0: #Flow from high index to low index cell
                                                    if ind_o_cell<j: #From j to ind_o_cell
                                                        if j-NwallsJun not in config.sym_zombie0:
                                                            matrix_C[j][j] -= Fcc
                                                        if ind_o_cell-NwallsJun not in config.sym_zombie0:
                                ...
                                if path == "membrane": #Membrane connection
                                    ...
                                    Flow = K * (psi - psin + s_membranes[jmb]*(Os_walls[i] - Os_cells[j-NwallsJun]))
                                    jmb+=1
                                    if ((j-NwallsJun not in config.intercellular_ids) and (j not in listxyl)) or Barrier==0:
                                        if Flow > 0 :
                                            UptakeLayer_plus[row][iMaturity][count] += Flow #grouping membrane flow rates in cell layers
                                        else:
                                            UptakeLayer_minus[row][iMaturity][count] += Flow
        ...
        for Maturity in Maturityrange:
            ...
            with open(newpath+"Macro_prop_"+str(Barrier)+","+str(iMaturity)+".txt", "a") as myfile:
                ...
                for j in range(int(r_discret[0].item())):
